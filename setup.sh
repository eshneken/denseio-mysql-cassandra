...
echo "[mysqld]" | tee -a /u01/mysql-config/my.cnf > /dev/null
echo "innodb_buffer_pool_size=10737418240" | tee -a /u01/mysql-config/my.cnf > /dev/null
...
sudo docker run --name cassandra -d -p 7000:7000 --cpus=24 --memory=50g -e CASSANDRA_BROADCAST_ADDRESS=PRIVATEIP -e CASSANDRA_SEEDS=192.168.0.226 -v /u01/cassandra-data:/var/lib/cassandra cassandra:3.0.27
